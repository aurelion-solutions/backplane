...
// ListResponse is the GET /policy-assessment-runs envelope.
type ListResponse struct {
	Items  []*AssessmentRun `json:"items"`
	Total  int              `json:"total"`
	Limit  int              `json:"limit"`
	Offset int              `json:"offset"`
}

// RegisterRoutes mounts the read-only assessment-run HTTP surface on g.
//
// No POST / PATCH / DELETE: assessment runs are created and updated
// by the worker policy-assessment action, not by external clients.
// Manual or ad-hoc API triggers will land on a dedicated trigger
// endpoint owned by the worker slice.
func RegisterRoutes(g *echo.Group, repo Repository) {
	g.GET("/policy-assessment-runs", listHandler(repo))
	g.GET("/policy-assessment-runs/:id", getHandler(repo))
}
...
func listHandler(repo Repository) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if s := c.QueryParam("scope_principal_id"); s != "" {
			id, err := uuid.Parse(s)
			if err != nil {
				return c.JSON(http.StatusBadRequest, errorBody(err))
			}
			f.ScopePrincipalID = &id
		}
		if s := c.QueryParam("scope_application_id"); s != "" {
			id, err := uuid.Parse(s)
			if err != nil {
				return c.JSON(http.StatusBadRequest, errorBody(err))
			}
			f.ScopeApplicationID = &id
		}

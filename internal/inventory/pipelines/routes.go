...
// ListResponse is the GET /pipelines envelope.
...
// RegisterRoutes mounts the read-only pipelines HTTP surface on g.
//
// No POST / PATCH / DELETE: every change to a pipeline comes from a
// cartridge edit picked up by the sync loop.
func RegisterRoutes(g *echo.Group, repo Repository) {
	g.GET("/pipelines", listHandler(repo))
	g.GET("/pipelines/:id", getHandler(repo))
}
